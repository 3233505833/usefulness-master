...
from utils.new_file_utils import load_jsonl, get_num_lines, check_file_and_mkdir_for_save,  save_jsonl, save_json, DataItem
...
class GPT3TextCLS(object):
    ...
    def step2_get_gpt3_results(self, query_save_path: str, unjudge_doc_path: str, resume: bool = False):
        thought_summary = False
        if self.config["data_name"] == "KDD":
            score_options = "1,2,3这三个打分"
        else:
            score_options = "0,1,2,3这四个打分"
        ...
        def format_document(item):
            ...
            return f"{item['text']}该文档是搜索者第{int(item['order']) + 1}个点击进入阅读的文档，该文档在文档列表中的排名是第{int(item['serp_id']) + 1}名，信息搜索者对该文档的阅读时间为{np.ceil(item['dwell_time'] / 1000)}秒，在查看这个文档之前搜索者已经获取的信息有：{his}"

        def example_format_document(item):
            return f"{item['text']}该示例文档是搜索者第{int(item['order']) + 1}个点击进入阅读的文档，该文档在文档列表中的排名是第{int(item['serp_id']) + 1}名，信息搜索者对该文档的阅读时间为{np.ceil(item['dwell_time'] / 1000)}秒。"
        ...
        def query_level_feature(item, df):
            ...
            prompt = (
                f"{item['text']}。该文档是搜索者第{int(item['order']) + 1}个点击进入阅读的文档，该文档在文档列表中的排名是第{int(item['serp_id']) + 1}名，"
                f"而搜索者在此查询中访问的文档总数量是 {int(round(row['PageVisit']))}个。"
                f"点击的文档的平均排名是 {round(row['AvgClickRank'], 1)}位。"
                f"点击的最深的文档是第 {int(round(row['ClickDepth']))}位。"
                f"信息搜索者对该文档的阅读时间为{np.ceil(item['dwell_time'] / 1000)}秒。"
                f"而搜索者在此查询中阅读每个文档的平均时间是 {round(row['AvgContent'] / 1000, 1)}秒，"
                f"另外，在此搜索会话中，这是第 {int(round(row['QueryOrder']))} 个查询，"
                f"{session_end_text}"
                f"搜索者在此查询上的总耗时是 {round(row['QueryDwellTime'] / 1000, 1)}秒。"
            )
            return prompt
        def give_example(examples:list):
            try:
                example=examples[0]
                example_str = example_format_document(example)
                example_str = "以下是有用性得分为4的最佳文档示例：" + example_str
            except:
                example_str=""
            return example_str
        ...
        task_ids_to_keep=[]
        example_item_lst = load_jsonl(query_save_path, offset=resume_offset)
        example_item_lst = [item for item in example_item_lst if item.get("task_id") in task_ids_to_keep]

        data_item_lst = load_jsonl(unjudge_doc_path, offset=resume_offset)
        data_item_lst = [item for item in data_item_lst if item.get("task_id") in task_ids_to_keep]
        ...
        for idx, batch_data_item in tqdm(enumerate(chunked(data_item_lst, num_workers)),
            ...
            for i, item in enumerate(batch_data_item):
                examples = [example for example in example_item_lst if example.get('query_id') == item['query_id']]
                example_str=give_example(examples)
                print(i)
                batch_prompt_text[i].append({"role": "user",
                                             "content": "现在的信息需求和查询为：" + item[
                                                 "prompt_text"] + "。\n\n然后我会提供给你文档，你必须从" + score_options + "中选择一个代表此文档对于当前查询和信息需求的有用性程度，数字越大代表有用性越高。"+example_str})
                batch_prompt_text[i].append({'role': 'assistant', 'content': '好的，请提供待判断的文档。'})
                if self.config["data_name"] == "KDD":
                    batch_prompt_text[i].append(
                        {'role': 'user', 'content': query_level_feature(item, df)})
                else:
                    batch_prompt_text[i].append({'role': 'user', 'content': format_document_new(item)})
                batch_prompt_text[i].append({'role': 'assistant', 'content': '已接待判断的文档。'})
                if thought_summary == False:
                    batch_prompt_text[i].append({"role": "user",
                                                 'content': "现在请你从" + score_options + "中选择一个代表该文档对于当前查询和信息需求的有用性打分，数字越大代表有用性越高。请先说出你的思考，然后再输出你打的分数，格式为：思考\n\n\n\n打分。"})
                else:
                    batch_prompt_text[i].append({"role": "user",
                                                 'content': "现在请你从" + score_options + "中选择一个代表该文档对于当前查询和信息需求的有用性打分，数字越大代表有用性越高。提示：你可以参考“有帮助”、“详细”、“相关”、“百科”、“具体”、“全面”这六个方面或其中几个方面进行考量，请先说出你的思考，然后再输出你打的分数，格式为：思考\n\n\n\n打分。"})
            ...
            for data_item, returned_text, returned_logprobs, batch_prompt_text_inputgpt in zip(batch_data_item, gpt_returned_text,
                ...
                save_result_item = {"gpt_returned_result": returned_text, "gold_label": data_item["gold_label"], "prompt_text": str(batch_prompt_text_inputgpt),
                                    ...
                                    "serp_id": data_item["serp_id"], "order": data_item["order"],"dwell_time":data_item["dwell_time"],"his":data_item["his"]
                                    }
    ...
    def step3_map_competition_result_to_label(self, step2_gpt_result_path: str):
        ...
        return saved_result_path_lst[0]
    def merge4_3210(self,step3_save_path,step1_save_path):
        ...
        output_file = os.path.join(self.config.save_log_dir, "merged_output.jsonl")

        merge_jsonl_files(step3_save_path,step1_save_path,output_file)
...
def run():
    ...
    if 2 in step_idx_lst:
        query_save_path = os.path.join(gpt3_for_text_cls_task.config.save_log_dir, "judge4.jsonl")
        unjudge_doc_path = os.path.join(gpt3_for_text_cls_task.config.save_log_dir, "unjudge4.jsonl")
        step2_save_path = gpt3_for_text_cls_task.step2_get_gpt3_results(query_save_path,unjudge_doc_path,
    ...
    if 3 in step_idx_lst:
        ...
        gpt3_for_text_cls_task.merge4_3210(os.path.join(gpt3_for_text_cls_task.config.save_log_dir, "judge4.jsonl"),step3_save_path)
